...
Cmax <- apply(Abun, 2, function(x) iNEXT.3D:::Chat.Ind(x, 2*sum(x))) %>% min %>% round(., 4)
Cmin <- apply(Abun, 2, function(x) iNEXT.3D:::Chat.Ind(x, sum(x))) %>% min %>% round(., 4)
...
Cmax <- apply(Inci_freq, 2, function(x) iNEXT.3D:::Chat.Sam(x, 2*x[1])) %>% min %>% round(., 4)
Cmin <- apply(Inci_freq, 2, function(x) iNEXT.3D:::Chat.Sam(x, x[1])) %>% min %>% round(., 4)
